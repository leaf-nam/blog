...
- 물론, 해당 라이브러리에서도 내부적으로는 [지난 시간](https://1eaf.site/posts/spring_security/5)에 구현한 `JWT 인증 필터`와 유사한 로직이 진행됩니다.

> 개인적으로는 필터를 직접 구현하는 것보다 **많은 사용자들에게서 이미 검증된 라이브러리를 사용**하는 것이 더욱 안전해 보이고, 예기치 못한 오류를 방지할 수 있을 것 같습니다.
